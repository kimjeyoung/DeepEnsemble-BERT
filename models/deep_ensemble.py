...
class Classifier(nn.Module):
    def __init__(self, backbone,
        ...
        # eps : input range of the corresponding dimension ~= 1%
        self.eps = torch.max(self.backbone.embeddings.word_embeddings.weight).item() * 0.01

    def get_adv(self, embedding_outputs, loss):
        device = self.device
        emb_grad = grad(loss, embedding_outputs, retain_graph=True)
        p_adv = torch.FloatTensor(self.eps * l2_normalize(emb_grad[0].data)).to(device)
...
class Regressor(Classifier):
    def __init__(self, backbone,
                 ...
                 device='cpu',
                 val_pos_cons=1e-6):
        super().__init__(backbone, hidden_size, num_classes, device)
